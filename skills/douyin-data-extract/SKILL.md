...
source: Hermes/scripts（cdp_extract.py + weekly_extract.py）
...
## 何时用
- 抓自己账号数据（播放/CTR/赞评藏享/粉丝），做作品分析
- 抓单条视频的正文、评论区、作品列表
- 打「自动抓数据 → 四层拆解 → 写作品分析报告」闭环

## 核心原理
- **CDP = Chrome DevTools Protocol**：复用本机已登录抖音的 Chrome（专用 profile），通过调试端口 9222 发指令。
...
| 脚本 | 抓什么 | 状态 |
|:--|:--|:--|
| `_check_new.py` | 粉丝数 + 最新作品列表（快查，~30秒）| ✅ 实测 |
| `_crawl_full.py` | 全量单条指标：播放/赞/评/藏/享/吸粉/完播/2秒跳出 → `_alldata.csv` | ✅ 实测 |
| `cdp_extract.py` | 单条视频（原始版，需按本机适配）| ⚠️ 硬编码路径 |
| `weekly_extract.py` | 创作者中心（原始版，需按本机适配）| ⚠️ 硬编码路径 |
...
```
> 两脚本只「连接已开的 Chrome(9222)」，不启动 Chrome、不 taskkill，安全。
